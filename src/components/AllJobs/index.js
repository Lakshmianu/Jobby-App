...
class AllJobs extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput} = this.state 
    const jobsApiUrl = `https://api.ccbp.in/jobs?employment_type=${checkboxInputs}&minimum_package=${radioInput}&search=${searchInput}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    ...
    if (responseJobs.ok === true) {
      const fetchDataJobs = await responseJobs.json()
      const updatedDataJobs = fetchDataJobs.jobs.map(eachItem => ({
